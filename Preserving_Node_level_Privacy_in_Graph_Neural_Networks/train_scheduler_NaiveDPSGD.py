...
class trainer:
    def __init__(self, *, model, optimizer, loaders, device, criterion, args):
        ...
        # cal = sampling.sr_calculator( 
        #         batch_size = self.args.expected_batchsize, 
        #         N = len(self.train_loader.dataset), 
        #         K = self.args.K, 
        #         max_in_degree = self.args.max_in_degree, 
        #         min_out_degree = self.args.min_out_degree,
        #         num_neighbors = self.args.num_neighbors,
        #         num_not_neighbors = self.args.num_not_neighbors,
        #     )
            
        # sr, more = cal.calculate()
        # args.q = sr
        # print(f'\n{"="*40}')
        # # print(f'One graph has nodes: {self.args.K * (self.args.num_neighbors + self.args.num_not_neighbors) + 1}')
        # print(f'==> sampling rate: {sr:.3f}, it is more than {more:.3f} times than the original {self.args.expected_batchsize/len(self.train_loader.dataset):.3f}')
        # print(f'{"="*40}\n')

        self.args.delta = 1/len(self.train_loader.dataset)**1.1
        self.std = self.args.std = aa.get_std(
                                        q = self.args.expected_batchsize/len(self.train_loader.dataset),
        ...
        self.json_recorder = json_data_recorder(f'jd_{self.train_loader.dataset.graph_data_name}_{self.args.graph_setting}.json')
    ...
    def one_epoch(self, train_or_val, loader):
        ...
        def manual_forward(x, targets):
            ...
            # targets = targets[:,0]
            # print(f'==> out shape: {out.shape}, targets shape: {targets.shape}')
            loss = self.criterion(out, targets.flatten())
        ...
        # self.write_log(f'    {train_or_val}: {self.record_data_type} = {float(metrics.__getattr__(self.record_data_type))*100:.2f}%' )
    ...
    def other_routine(self, per_grad):
        ...
        for p_model, p_per in zip(self.model.parameters(), per_grad):
            ...
            # noise_std = 3
            p_model.grad = p_model.grad + torch.randn_like(p_model.grad) * self.std * self.args.C / self.args.expected_batchsize
    ...
    def clip_per_grad(self, per_grad):
        ...
        for index, p in enumerate(per_grad):
            ''' normalizing '''
            per_grad[index] = p / self._make_broadcastable(per_grad_norm / self.args.C, p) 
            ''' clipping '''
